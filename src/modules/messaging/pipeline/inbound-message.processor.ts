...
import { ZapiContactEnricherService } from '../../channel-hub/adapters/zapi/zapi-contact-enricher.service';
...
export class InboundMessageProcessor extends WorkerHost {
  ...
  constructor(
    ...
    private readonly zapiEnricher: ZapiContactEnricherService,
  ...
  private async handle(job: Job<InboundJobData | StatusJobData>): Promise<any> {
    ...
    try {
      ...
      // WhatsApp via Z-API: same lazy-enrich pattern.
      if (message.channelType === ChannelType.WHATSAPP_ZAPI) {
        const [channel, contact] = await Promise.all([
          this.prisma.channel.findUnique({ where: { id: channelId } }),
          isNewContact
            ? Promise.resolve(null)
        ...
        if (channel && (isNewContact || !contact?.avatarUrl)) {
          this.zapiEnricher
            .enrich(channel, message.externalContactId)
            .catch((err) =>
              this.logger.warn(`Z-API enrichment failed: ${err.message}`),
            );
        }
      }
