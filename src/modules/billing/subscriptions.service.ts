import { Injectable, Logger, NotFoundException } from '@nestjs/common';
import { Prisma, SubscriptionStatus } from '@prisma/client';
import { PrismaService } from '../../database/prisma.service';

const TRIAL_DAYS = 7;
const DEFAULT_TRIAL_PLAN = 'STARTER';

@Injectable()
export class SubscriptionsService {
  private readonly logger = new Logger(SubscriptionsService.name);

  constructor(private readonly prisma: PrismaService) {}

  /**
   * Cria subscription em status TRIAL pra uma organization recém-criada.
   * 7 dias sem cartão · plan SOLO default · upgrade depois via Kirvano (PR 2.2).
   * Idempotente · se já existe, retorna a existente.
   */
  async createTrialForOrg(organizationId: string) {
    const existing = await this.prisma.subscription.findUnique({
      where: { organizationId },
    });
    if (existing) return existing;

    const trialEndsAt = new Date();
    trialEndsAt.setDate(trialEndsAt.getDate() + TRIAL_DAYS);

    const sub = await this.prisma.subscription.create({
      data: {
        organizationId,
        planCode: DEFAULT_TRIAL_PLAN,
        status: SubscriptionStatus.TRIAL,
        trialEndsAt,
      },
    });
    this.logger.log(`Trial subscription created for org ${organizationId} · ends ${trialEndsAt.toISOString()}`);
    return sub;
  }

  /**
   * Cria o trial DENTRO de uma transação existente, usando o client `tx`.
   * Pensado pro fluxo de cadastro: o register roda em PrismaSystemService
   * (role que bypassa RLS), então a subscription nasce junto com user/org de
   * forma atômica e confiável — sem fire-and-forget e sem depender de
   * findOrCreate posterior. Política MVP grátis = TRIAL_DAYS (7) dias.
   */
  async createTrialInTx(tx: Prisma.TransactionClient, organizationId: string) {
    const trialEndsAt = new Date();
    trialEndsAt.setDate(trialEndsAt.getDate() + TRIAL_DAYS);
    return tx.subscription.create({
      data: {
        organizationId,
        planCode: DEFAULT_TRIAL_PLAN,
        status: SubscriptionStatus.TRIAL,
        trialEndsAt,
      },
    });
  }

  /** Subscription da org com plano populado. Lança 404 se não existe. */
  async findByOrg(organizationId: string) {
    const sub = await this.prisma.subscription.findUnique({
      where: { organizationId },
      include: { plan: true },
    });
    if (!sub) {
      throw new NotFoundException(`No subscription for org ${organizationId}`);
    }
    return sub;
  }

  /** Encontra ou cria · útil pra orgs antigas que não tinham subscription. */
  async findOrCreateForOrg(organizationId: string) {
    try {
      return await this.findByOrg(organizationId);
    } catch {
      const sub = await this.createTrialForOrg(organizationId);
      return this.findByOrg(sub.organizationId);
    }
  }

  /**
   * Status simplificado da conta · usado pelo banner do dashboard e guard de rotas.
   * Regra de acesso (modelo trial-first):
   * - liberado: status ACTIVE, ou TRIAL com trialEndsAt ainda no futuro;
   * - suspenso: TRIAL expirado, PAST_DUE, CANCELED, EXPIRED, sem assinatura
   *   ou status desconhecido.
   * O trial dá acesso ANTES do pagamento (pilotos/founder/degustação).
   */
  async getAccountStatus(organizationId: string): Promise<{
    suspended: boolean;
    reason:
      | 'trial_active'
      | 'trial_expired'
      | 'past_due'
      | 'canceled'
      | 'expired'
      | 'no_subscription'
      | null;
    status: SubscriptionStatus | null;
    trialEndsAt: Date | null;
    planCode: string | null;
  }> {
    const sub = await this.prisma.subscription.findUnique({
      where: { organizationId },
    });
    if (!sub) {
      return {
        suspended: true,
        reason: 'no_subscription',
        status: null,
        trialEndsAt: null,
        planCode: null,
      };
    }

    // Modelo trial-first: ACTIVE e TRIAL válido (trialEndsAt no futuro) liberam
    // acesso. TRIAL expirado e os demais status bloqueiam.
    const trialValid =
      sub.status === SubscriptionStatus.TRIAL &&
      sub.trialEndsAt != null &&
      sub.trialEndsAt.getTime() > Date.now();

    let suspended: boolean;
    let reason:
      | 'trial_active'
      | 'trial_expired'
      | 'past_due'
      | 'canceled'
      | 'expired'
      | null = null;

    switch (sub.status) {
      case SubscriptionStatus.ACTIVE:
        suspended = false;
        break;
      case SubscriptionStatus.TRIAL:
        suspended = !trialValid;
        reason = trialValid ? 'trial_active' : 'trial_expired';
        break;
      case SubscriptionStatus.PAST_DUE:
        suspended = true;
        reason = 'past_due';
        break;
      case SubscriptionStatus.CANCELED:
        suspended = true;
        reason = 'canceled';
        break;
      case SubscriptionStatus.EXPIRED:
        suspended = true;
        reason = 'expired';
        break;
      default:
        // status desconhecido/inválido: bloqueia por segurança
        suspended = true;
        break;
    }

    return {
      suspended,
      reason,
      status: sub.status,
      trialEndsAt: sub.trialEndsAt,
      planCode: sub.planCode,
    };
  }
}
